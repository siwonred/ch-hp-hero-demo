...
class World {
  ...
  renderParticle(particle) {
    ...
    const dThreshold = Math.max(window.innerWidth, window.innerHeight) / 2;
    var alpha = 1;
    // if (d > dThreshold + 1) {

    //   const ex = Math.exp(- d / 100 - dThreshold);
    //   alpha = ex / (ex + 1);

    //   // alpha = 1 / ((d - dThreshold) / 200);
    // }
    const ex = Math.exp(- (d - dThreshold) / 400.0); // sigmoid
    alpha = ex / (ex + 1);

    // alpha = alpha * (1 - this.progress);
